...
bot.command("start", async (ctx) => {
    ...
    await ctx.reply("Witaj na bocie Gornika! 👋\n\nWybierz jedną z opcji poniżej, aby dowiedzieć się więcej o naszych usługach.", { reply_markup: keyboard });
});
bot.callbackQuery(/^info_(\d+)$/, async (ctx) => {
    ...
    await ctx.reply(`📌 *${page.name}*\n\n${page.description}\n\n🔗 ADRES: \`${page.link}\``, { parse_mode: "Markdown" });
});
